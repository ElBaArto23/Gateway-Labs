...
resource roleAssignmentCognitiveServicesUser 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for (config, i) in aiServicesConfig: if (length(apimPrincipalId) > 0) {
  scope: cognitiveServices[i]
  // El nombre del role assignment (su GUID) debe incluir apimPrincipalId. Si no lo incluye,
  // cambiar de identidad (ej. migrar de una APIM compartida a otra) genera el MISMO guid que
  // ya existe pero con un principalId distinto, y Azure rechaza esa "actualización" con
  // RoleAssignmentUpdateNotPermitted (el principalId/scope/roleDefinitionId de un role
  // assignment son inmutables una vez creado). Incluir apimPrincipalId en el guid hace que un
  // principal nuevo cree un role assignment nuevo en vez de intentar mutar el viejo.
  name: guid(subscription().id, resourceGroup().id, config.name, cognitiveServicesUserRoleDefinitionID, apimPrincipalId)
    properties: {
