...
resource apiDiagnosticsAppInsights 'Microsoft.ApiManagement/service/apis/diagnostics@2022-08-01' = if (!empty(appInsightsId) && !empty(appInsightsInstrumentationKey) && !empty(apimLoggerId)) {
  ...
  properties: {
    ...
    logClientIp: true
    // Reuses the same logger passed in via apimLoggerId, instead of guessing a fixed
    // logger name ('appinsights-logger') that only existed when this module created its
    // own dedicated APIM. On a shared APIM the caller's logger has a lab-specific name.
    loggerId: apimLoggerId
    metrics: true
    verbosity: 'verbose'
    sampling: {
