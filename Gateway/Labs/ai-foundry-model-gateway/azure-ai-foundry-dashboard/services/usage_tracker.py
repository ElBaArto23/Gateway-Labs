...
def get_usage_records(
    ...
    """
    Devuelve los registros de USAGE_LOG_PATH tal cual, uno por cada
    llamada individual (sin agregar por modelo). Útil para responder
    "¿cuál llamada costó/consumió más?", a diferencia de
    get_consumption_summary() que agrupa por modelo.
    """
...
def get_consumption_summary(
    ...
    Args:
        agent_name: si se pasa, filtra solo ese agente.
        since: si se pasa, filtra solo registros con timestamp >= since
               (debe ser timezone-aware; si es naive se asume UTC).

    Returns:
        {
            "agent_name": <filtro aplicado o None>,
            "since": <filtro aplicado o None>,
            "call_count": int,
            "total_tokens": int,
            "total_cost": float,
...
def check_budget(
    ...
    Raises:
        BudgetExceededError: si se supera max_cost_usd o max_tokens.

    NOTA (limitación conocida): esto relee el archivo completo en cada
    llamada y no tiene protección real contra condiciones de carrera
    entre procesos/instancias -- es un sustituto casero de lo que
    azure-openai-token-limit hace en policy.xml a nivel de gateway.
    Para producción multi-tenant, esto se reemplaza por rate limiting
    real en APIM (ver pendiente de migración a BYOM).
    """
